...
const serviceAccount = require('../lib/firebaseAdminKey.json');
admin.initializeApp({
...
const productImages = {
  ...
  'exotech-gel': {
    ...
    video: '/videos/exotech-gel-demo.webm'
  },
  'exosignal-spray': {
    images: [
    ...
    video: '/videos/exosignal-spray-demo.webm'
  },
  'pdrn-serum': {
    images: [
    ...
    video: '/videos/pdrn-serum-demo.webm'
  },
  'stem-cell-activator': {
    images: [
    ...
    video: '/videos/stem-cell-activator-demo.webm'
  }
};
...
async function uploadImageToStorage(imagePath, destinationPath) {
  try {
    ...
    if (!fs.existsSync(localPath)) {
      console.log(`Local file ${localPath} does not exist, skipping...`);
      return null;
    }
    ...
    console.log(`Uploaded ${imagePath} to ${destinationPath}`);
    return `gs://${bucket.name}/${destinationPath}`;
  } catch (error) {
...
async function uploadVideoToStorage(videoPath, destinationPath) {
  try {
    const file = bucket.file(destinationPath);
    const [exists] = await file.exists();
    
    if (exists) {
      console.log(`Video ${destinationPath} already exists, skipping...`);
      return `gs://${bucket.name}/${destinationPath}`;
    }
    ...
    if (!fs.existsSync(localPath)) {
      console.log(`Local video ${localPath} does not exist, skipping...`);
      return null;
    }
    ...
    console.log(`Uploaded ${videoPath} to ${destinationPath}`);
    return `gs://${bucket.name}/${destinationPath}`;
  } catch (error) {
...
async function createProductImages() {
  ...
  for (const [slug, productData] of Object.entries(productImages)) {
    for (const image of productData.images) {
      const destinationPath = `products/${slug}/${path.basename(image.url)}`;
      imagePromises.push(uploadImageToStorage(image.url, destinationPath));
    }
    
    if (productData.video) {
      const destinationPath = `products/${slug}/${path.basename(productData.video)}`;
      imagePromises.push(uploadVideoToStorage(productData.video, destinationPath));
    }
  }
...
async function main() {
  try {
    console.log('Starting product image upload and database update...');
